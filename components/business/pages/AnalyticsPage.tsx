...
export function AnalyticsPage() {
  ...
  return (
    <div className="business-page analytics-page">
      <BusinessPageHeader eyebrow="گزارش عملکرد" title="گزارش‌ها" description="شاخص‌های عملیاتی بر اساس شعبه و بازهٔ انتخابی محاسبه می‌شوند." />
      ...
      <section className="business-metrics analytics-metrics"><MetricCard label="درآمد" value={formatMoney(revenue)} hint="فروش ناخالص" tone="brand" /><MetricCard label="سفارش" value={formatNumber(orderCount)} hint="در بازه انتخابی" /><MetricCard label="نرخ فروش" value={`${formatNumber(total ? Math.round(sold / total * 100) : 0)}٪`} hint={`${formatNumber(sold)} از ${formatNumber(total)} بسته`} /><MetricCard label="بسته نجات‌یافته" value={formatNumber(saved)} hint={`${formatDecimal(saved * 0.78)} کیلو غذای برآوردی`} /><MetricCard label="میانگین سفارش" value={formatMoney(orderCount ? Math.round(revenue / orderCount) : 0)} hint="بر پایه فروش ثبت‌شده" /><MetricCard label="عدم مراجعه" value={`${formatDecimal(orderCount ? noShows / orderCount * 100 : 0)}٪`} hint={`${formatNumber(noShows)} مورد`} /><MetricCard label="بازپرداخت" value={`${formatDecimal(orderCount ? refunds / orderCount * 100 : 0)}٪`} hint={`${formatNumber(refunds)} مورد`} /><MetricCard label="میانگین امتیاز" value={formatDecimal(rating)} hint="از ۵" /><MetricCard label="نرخ پیگیری" value={`${formatDecimal(orders.length ? complaintCount / orders.length * 100 : 0)}٪`} hint={`${formatNumber(complaintCount)} مورد`} /></section>
      <div className="analytics-grid">
        <section className="business-panel chart-card wide"><h2>درآمد در طول زمان</h2><p>تومان</p><MiniBarChart money title="درآمد در طول زمان" values={chartDays.map((item) => item.revenue)} labels={chartDays.map((item) => item.date.slice(8))} /></section>
