...
export function SettingsPage() {
  ...
  return (
    <div className="business-page settings-page">
      <BusinessPageHeader eyebrow="تنظیمات" title="مدیریت مجموعه" description="اطلاعات، شعب، کارکنان و دسترسی‌های کافه ویونا را مدیریت کنید." />
      <div className="settings-layout"><nav className="settings-tabs" aria-label="بخش‌های تنظیمات">{settingsTabs.map((item) => <button type="button" key={item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}>{item.label}<Icon name="chevron" /></button>)}</nav><section className="business-panel settings-content">
        ...
        {tab === "hours" && <div><SettingsTitle title="ساعات کاری هفتگی" text="یک الگوی کاری برای روزهای هفته تنظیم کنید." /><div className="weekly-hours">{["شنبه", "یکشنبه", "دوشنبه", "سه‌شنبه", "چهارشنبه", "پنجشنبه", "جمعه"].map((day) => <div key={day}><strong>{day}</strong><label className="inline-switch"><input type="checkbox" defaultChecked={day !== "جمعه"} disabled={!ownerOnly} /><span /></label><input type="time" defaultValue="08:00" disabled={!ownerOnly || day === "جمعه"} /><span>تا</span><input type="time" defaultValue="22:00" disabled={!ownerOnly || day === "جمعه"} /></div>)}</div><button className="business-primary" type="button" disabled={!ownerOnly} onClick={() => notify("ساعات کاری ذخیره شد.")}>ذخیره ساعات</button></div>}
        ...
        {tab === "notifications" && <div><SettingsTitle title="اعلان‌ها" text="رویدادهای قابل نمایش در مرکز اعلان را انتخاب کنید." /><div className="notification-settings">{Object.entries({ order: "سفارش تازه", stock: "کمبود موجودی", review: "نظر و پیگیری کیفیت", settlement: "تسویه مالی" }).map(([id, label]) => <label key={id}><span><strong>{label}</strong><small>نمایش در مرکز اعلان پنل</small></span><span className="inline-switch"><input type="checkbox" checked={notifications[id as keyof typeof notifications]} onChange={() => setNotifications({ ...notifications, [id]: !notifications[id as keyof typeof notifications] })} /><i /></span></label>)}</div><button className="business-primary" type="button" onClick={() => notify("تنظیمات اعلان‌ها ذخیره شد.")}>ذخیره اعلان‌ها</button></div>}
        {tab === "finance" && (can("finance:read") ? <div><SettingsTitle title="اطلاعات مالی" text="حساب مقصد تسویه‌های مجموعه" /><div className="demo-bank-card"><Icon name="info" /><span><small>شماره شبای ثبت‌شده</small><strong dir="ltr">{state.business.demoBankIban}</strong></span></div><p>برای تغییر حساب تسویه، یک درخواست پشتیبانی ثبت کنید.</p><Link className="business-secondary" href="/business/support">درخواست تغییر حساب</Link></div> : <EmptyBusinessState title="دسترسی مالی ندارید" text="فقط مالک و حسابدار می‌توانند این بخش را ببینند." />)}
        {tab === "documents" && <div><SettingsTitle title="مدارک" text="مدارک مجموعه را برای دسترسی سریع نگه‌داری کنید." /><label className="document-drop"><Icon name="share" /><strong>{documentName || "یک مدرک انتخاب کنید"}</strong><small>PDF یا تصویر تا ۲ مگابایت</small><input type="file" accept="application/pdf,image/*" disabled={!ownerOnly} onChange={(event) => { const file = event.target.files?.[0]; if (file) { setDocumentName(file.name); notify("مدرک اضافه شد."); } }} /></label>{documentName && <div className="document-preview"><span>PDF</span><strong>{documentName}</strong><button type="button" onClick={() => setDocumentName("")} aria-label="حذف مدرک"><Icon name="trash" /></button></div>}</div>}
        {tab === "security" && <SecuritySettings ownerOnly={ownerOnly} notify={notify} />}
      </section></div>

      {branchDialog && <DialogShell titleId="branch-dialog-title" onClose={() => setBranchDialog(false)}><form className="business-dialog compact-form" onSubmit={(event) => { event.preventDefault(); addBranch(branchForm); setBranchDialog(false); notify("شعبه اضافه شد."); }}><p className="eyebrow">شعبه تازه</p><h2 id="branch-dialog-title">اطلاعات شعبه</h2><label><span>نام شعبه</span><input required value={branchForm.name} onChange={(event) => setBranchForm({ ...branchForm, name: event.target.value })} /></label><label><span>منطقه</span><input required value={branchForm.area} onChange={(event) => setBranchForm({ ...branchForm, area: event.target.value })} /></label><label><span>نشانی</span><textarea required rows={3} value={branchForm.address} onChange={(event) => setBranchForm({ ...branchForm, address: event.target.value })} /></label><label><span>تلفن</span><input value={branchForm.phone} onChange={(event) => setBranchForm({ ...branchForm, phone: event.target.value })} /></label><button className="business-primary full" type="submit">افزودن شعبه</button></form></DialogShell>}
      {staffDialog && <DialogShell titleId="staff-dialog-title" onClose={() => setStaffDialog(false)}><form className="business-dialog compact-form" onSubmit={(event) => { event.preventDefault(); addStaff({ ...staffForm, permissions: rolePermissions[staffForm.role], active: true }); setStaffDialog(false); notify("کارمند اضافه شد."); }}><p className="eyebrow">عضو تازه</p><h2 id="staff-dialog-title">اطلاعات کارمند</h2><label><span>نام و نام خانوادگی</span><input required value={staffForm.name} onChange={(event) => setStaffForm({ ...staffForm, name: event.target.value })} /></label><label><span>موبایل</span><input required value={staffForm.mobile} onChange={(event) => setStaffForm({ ...staffForm, mobile: event.target.value })} /></label><label><span>شعبه</span><select value={staffForm.branchId} onChange={(event) => setStaffForm({ ...staffForm, branchId: event.target.value })}>{state.branches.map((branch) => <option value={branch.id} key={branch.id}>{branch.name}</option>)}</select></label><label><span>نقش</span><select value={staffForm.role} onChange={(event) => setStaffForm({ ...staffForm, role: event.target.value as StaffRole })}>{Object.entries(staffRoleLabel).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></label><button className="business-primary full" type="submit">افزودن کارمند</button></form></DialogShell>}
    </div>
  );
}
...
function SecuritySettings({ ownerOnly, notify }: { ownerOnly: boolean; notify: (message: string, kind?: "success" | "error") => void }) {
  ...
  const save = (event: FormEvent) => { event.preventDefault(); if (passwords.next.length < 8) return notify("رمز باید حداقل ۸ نویسه باشد.", "error"); if (passwords.next !== passwords.repeat) return notify("تکرار رمز یکسان نیست.", "error"); setPasswords({ current: "", next: "", repeat: "" }); notify("رمز تغییر کرد."); };
  return <div><SettingsTitle title="امنیت" text="رمز ورود و نشست‌های فعال را مدیریت کنید." /><form className="settings-form security-form" onSubmit={save}><label><span>رمز فعلی</span><input type="password" required value={passwords.current} onChange={(event) => setPasswords({ ...passwords, current: event.target.value })} disabled={!ownerOnly} /></label><label><span>رمز تازه</span><input type="password" minLength={8} required value={passwords.next} onChange={(event) => setPasswords({ ...passwords, next: event.target.value })} disabled={!ownerOnly} /></label><label><span>تکرار رمز</span><input type="password" minLength={8} required value={passwords.repeat} onChange={(event) => setPasswords({ ...passwords, repeat: event.target.value })} disabled={!ownerOnly} /></label><button className="business-primary" type="submit" disabled={!ownerOnly}>تغییر رمز</button></form><div className="signout-card"><div><strong>خروج از پنل</strong><small>به صفحه انتخاب نوع ورود برمی‌گردید.</small></div><Link href="/">خروج</Link></div></div>;
}
