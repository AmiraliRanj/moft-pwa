...
export function OrdersPage() {
  ...
  return (
    <div className="business-page orders-page">
      ...
      {confirmAction && <DialogShell label="تأیید تغییر وضعیت سفارش" onClose={() => setConfirmAction(null)} size="center"><div className="cancel-dialog"><span className="danger-icon"><Icon name="info" /></span><h2>{confirmAction.status === "cancelled" ? "سفارش لغو شود؟" : "عدم مراجعه ثبت شود؟"}</h2><p>{confirmAction.status === "cancelled" ? "در صورت مجاز بودن، موجودی برگردانده و تراکنش بازپرداخت می‌شود." : "این وضعیت نشان می‌دهد مشتری در بازه دریافت مراجعه نکرده است."}</p><div><button className="secondary-button" type="button" onClick={() => setConfirmAction(null)}>انصراف</button><button className="danger-button" type="button" onClick={() => { changeStatus(confirmAction.order, confirmAction.status); setConfirmAction(null); }}>تأیید</button></div></div></DialogShell>}
    </div>
  );
}
