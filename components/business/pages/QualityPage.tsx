...
export function QualityPage() {
  ...
  return (
    <div className="business-page quality-page">
      ...
      <section className="business-metrics quality-metrics"><MetricCard label="امتیاز کلی" value={`${formatDecimal(average)} از ۵`} hint={`${formatNumber(branchReviews.length)} نظر`} tone="brand" /><MetricCard label="کیفیت تجربه" value={`${formatNumber(qualityScore)} از ۱۰۰`} hint="شاخص ترکیبی نمایشی" /><MetricCard label="پیگیری باز" value={formatNumber(open)} hint={`${formatNumber(complaints.length)} مورد کل`} tone={open ? "accent" : "default"} /><MetricCard label="پاسخ‌داده‌شده" value={formatNumber(branchReviews.filter((item) => item.response).length)} hint="پاسخ قابل مشاهده مشتری" /></section>
