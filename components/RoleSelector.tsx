...
export default function RoleSelector() {
  return (
    <main className="relative min-h-svh overflow-hidden bg-canvas px-3.5 sm:px-6 py-5 sm:py-7 max-w-[1200px] mx-auto flex flex-col justify-between">
      ...
      <header className="relative z-10 w-full max-w-[940px] mx-auto flex items-center justify-between">
        <BrandMark subtitle="غذای خوب، نزدیک تو" />
        <ThemeToggle compact />
      </header>

      <section className="relative z-10 w-full max-w-[940px] mx-auto mt-6 sm:mt-10 mb-4 sm:mb-6 text-start" aria-labelledby="role-title">
      ...
      <section id="role-options" className="relative z-10 w-full max-w-[940px] mx-auto grid grid-cols-2 gap-3 sm:gap-4" aria-label="انتخاب نوع ورود">
        <Link className="group relative flex min-h-[265px] sm:min-h-[310px] flex-col justify-between overflow-hidden rounded-2xl sm:rounded-3xl border border-line bg-surface p-4 sm:p-6 text-ink shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-line-strong hover:shadow-md active:scale-[0.985] no-underline" href="/customer">
          ...
          <span className="relative z-10 flex flex-1 flex-col text-start">
            <strong className="mb-2 sm:mb-3 block text-base sm:text-xl font-extrabold leading-tight text-ink text-start">برای خودم</strong>
          ...
          <span className="relative z-10 mt-auto flex min-h-[38px] sm:min-h-[44px] items-center justify-between gap-2 rounded-xl sm:rounded-2xl bg-[#174d3d] dark:bg-brand-2 dark:text-zinc-900 px-3 sm:px-4 text-xs sm:text-sm font-bold text-white transition-opacity group-hover:opacity-95">
